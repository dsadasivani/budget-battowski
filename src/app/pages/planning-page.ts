    ...
    @if (store.showPageSkeleton()) {
      ...
      <section class="page mobile-planning-page">
        ...
        <section class="stat-grid" [class.three]="!store.hasMonthlyReviewRows()">
          ...
          <article class="stat-card monthly-budget-card">
          ...
          <article class="stat-card planned-spend-card">
            <span class="icon-chip orange"><mat-icon aria-hidden="true">assignment</mat-icon></span>
            <p>Planned Spend</p>
            <strong>{{
              store.outflowTotal() + store.investmentTotal()
                | currency: 'INR' : 'symbol' : '1.0-0' : 'en-IN'
            }}</strong>
          </article>
          <article class="stat-card buffer-card">
            <span class="icon-chip green"
              ><mat-icon aria-hidden="true">verified_user</mat-icon></span
            >
        ...
        <section class="content-grid two-one">
          <div class="panel-stack">
            ...
            <article class="panel-card mobile-one-time-plans">
              ...
              <div
                class="data-table-wrap compact"
                tabindex="0"
                aria-label="One-time planned expenses table"
              >
                  ...
                  <tbody>
                    @for (expense of store.oneTimePlannedRows(); track expense.id) {
                      <tr>
                        ...
                        <td>
                          <b>{{
                            expense.amount | currency: 'INR' : 'symbol' : '1.0-0' : 'en-IN'
                          }}</b>
                        </td>
                        <td>
                          <span class="badge success">{{ expense.status }}</span>
            ...
            <article class="panel-card">
              ...
              <div class="allocation-bar" aria-hidden="true">
                @for (category of store.budgetAllocationRows(); track category.id) {
                  <span
                    [style.width.%]="store.clampPercent(category.share)"
