            ...
            <div class="soft-list">
              @for (expense of store.oneTimeEntries().slice(0, 4); track expense.id) {
                <article class="list-row">
                  ...
                  <div>
                    <strong>{{ expense.name }}</strong>
                    <small>{{ store.shortDateLabel(store.recordDate(expense)) }} &middot; {{ store.categoryName(expense.categoryId) }}</small>
                  </div>
                  <b>{{ expense.amount | currency: 'INR' : 'symbol' : '1.0-0' : 'en-IN' }}</b>
                </article>
              } @empty {
          ...
          <article class="panel-card mobile-investments-panel">
            ...
            <div class="soft-list compact">
              @for (investment of store.portfolioRows().slice(0, 4); track investment.id) {
                <article class="list-row">
                  <span class="icon-chip teal"><mat-icon aria-hidden="true">show_chart</mat-icon></span>
                  <div>
                    <strong>{{ investment.name }}</strong>
                    <small>{{ store.investmentFrequencyLabel(investment) }} &middot; {{ investment.memberName }}</small>
                  </div>
                  <b class="teal-text">{{ investment.monthlyAmount | currency: 'INR' : 'symbol' : '1.0-0' : 'en-IN' }}</b>
                </article>
              } @empty {
