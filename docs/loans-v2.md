...
Lender matching asks for the date, interest amount, and closing principal from a repayment-schedule EMI row, with a second row available for a stronger match. An optional dated part-payment may be entered or reused from the event ledger. The matcher evaluates every supported method, day basis, precision, and midpoint-rounding combination without using the lender name. Every supplied row must match the same policy. Results show each calculated value, the total rupee difference, whether multiple rules remain possible, and the recommended rule before it is applied. A newly entered matching part-payment is recorded as a loan event only when the account is saved. Accepted lender rows are saved as dated reconciliation evidence, so an exact match changes the account from **Estimated** to **Reconciled through** the latest matched date.
